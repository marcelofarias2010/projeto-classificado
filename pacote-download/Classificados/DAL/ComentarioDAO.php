...
class ComentarioDAO {
    ...
    public function RetornarUltmosClassificado(int $classificadocod) {
        try {
            $sql = "SELECT c.cod, c.mensagem, c.data, u.nome, c.comentario_cod FROM comentario c INNER JOIN usuario u ON u.cod = c.usuario_cod WHERE c.classificado_cod = :clacod AND c.status = 1 ORDER BY c.data DESC LIMIT 15";
            $param = array(
    ...
    public function RetornarTodosClassificadoCod(int $classificadocod) {
        try {
            $sql = "SELECT c.cod, c.status, c.mensagem, c.data, u.nome, u.email, c.comentario_cod FROM comentario c INNER JOIN usuario u ON u.cod = c.usuario_cod WHERE c.classificado_cod = :clacod ORDER BY c.data DESC";
            $param = array(
                ":clacod" => $classificadocod
            );
